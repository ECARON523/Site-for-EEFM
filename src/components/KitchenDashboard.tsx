...
import { Clock, Crown, CheckCircle2, ChefHat, Archive, ExternalLink, History, Filter } from 'lucide-react';
...
export default function KitchenDashboard() {
  const { orders, updateOrderStatus, updateOrderItemStatus } = useOrderStore();
  
  const [viewMode, setViewMode] = useState<ViewMode>('active');
  const [selectedDept, setSelectedDept] = useState<DepartmentFilter>('all');

  const baseOrders = orders.filter(o => viewMode === 'active' ? o.status !== 'archived' : o.status === 'archived');
  ...
  // Функция переключения статуса блюда по кругу
            ...
            <div key={order.id} className={cn("bg-bg-surface rounded-2xl flex flex-col overflow-hidden transition-all shadow-xl relative", (order.isVip && viewMode === 'active') ? "ring-2 ring-yellow-400 shadow-[0_0_20px_rgba(250,204,21,0.15)] ring-offset-2 ring-offset-bg-primary" : "border border-border-color", order.status === 'ready' && "opacity-80 scale-[0.99]", order.status === 'archived' && "opacity-60 grayscale-[30%] border-dashed border-2")}>
              ...
              <div className={cn("p-5 relative transition-colors", order.status === 'pending' ? "bg-yellow-400 text-black" : order.status === 'cooking' ? "bg-blue-600 text-white" : order.status === 'ready' ? "bg-green-600 text-white" : "bg-bg-surface-light text-text-muted border-b border-border-color")}>
                ...
                <div className="flex justify-between items-center mb-1">
                  <span className="text-sm font-mono font-bold opacity-80">#{order.id.split('-')[1].toUpperCase()}</span>
                  <span className={cn("text-sm font-black flex items-center gap-1.5 px-2.5 py-1 rounded-md", (order.status === 'pending' || order.status === 'archived') ? "bg-black/10" : "bg-black/20")}>
                    <Clock className="w-4 h-4" />{new Date(order.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                  </span>
                </div>
                <div className={cn("font-black text-2xl truncate pr-20", order.status === 'archived' && "text-text-primary")}>{order.userName}</div>
              </div>

              {/* СПИСОК БЛЮД (ТЕПЕРЬ С КНОПКАМИ) */}
                  ...
                  <div key={idx} className={cn("flex justify-between items-center gap-4 py-3 border-b border-border-color/50 last:border-0", item.status === 'ready' && "opacity-50")}>
                    ...
                    <div className="flex-1">
                      <Link to={`/recipe/${item.recipeId}`} className={cn("font-bold text-base leading-tight group flex items-start gap-2 transition-colors mb-1.5", order.status === 'archived' ? "text-text-primary" : "text-text-primary hover:text-primary")} title="Открыть рецепт">
                        <span className={cn("group-hover:underline underline-offset-4 line-clamp-2", item.status === 'ready' && "line-through")}>{item.title}</span>
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      <div className="font-black text-xl text-text-primary">x{item.quantity}</div>
                      
                      {/* КНОПКА СТАТУСА БЛЮДА */}
                      {order.status !== 'archived' && (
                        <button 
                          onClick={() => handleItemClick(order.id, item.recipeId, item.status)}
                          className={cn(
                            "w-12 h-12 rounded-xl flex items-center justify-center transition-all border-2 active:scale-95",
                            item.status === 'pending' ? "bg-bg-surface-light border-yellow-500 text-yellow-500 hover:bg-yellow-500/10 shadow-[0_0_10px_rgba(250,204,21,0.1)]" :
                            item.status === 'cooking' ? "bg-blue-600 border-blue-600 text-white shadow-[0_0_15px_rgba(59,130,246,0.4)] animate-pulse" :
                            "bg-green-600 border-green-600 text-white shadow-md"
                          )}
                          title={item.status === 'pending' ? 'Взять в работу' : item.status === 'cooking' ? 'Отметить готовым' : 'Отменить готовность'}
                        >
              ...
              {/* ПОДВАЛ ЧЕКА (Для Кассира) */}
              <div className="p-4 bg-bg-surface border-t border-border-color">
                
                {/* Кассир видит кнопку выдачи, если ВЕСЬ заказ готов */}
                ...
                {/* Если заказ еще не готов (и мы на кассе), показываем прогресс */}
                {selectedDept === 'all' && order.status !== 'ready' && order.status !== 'archived' && (
                  <div className="w-full py-3 rounded-xl bg-bg-surface-light border border-border-color flex items-center justify-center gap-2">
                    <span className="text-sm font-bold text-text-muted uppercase tracking-widest">
                      Готово блюд: <span className="text-primary font-black text-lg ml-1">{order.items.filter(i => i.status === 'ready').length} / {order.items.length}</span>
                    </span>
                  </div>
                )}

                {/* Если это повар в своем цеху, просто пишем его цех снизу (для красоты) */}
