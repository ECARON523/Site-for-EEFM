...
export default function RecipeCard({ recipe }: RecipeCardProps) {
  ...
  // ВОЗВРАЩАЕМ УМНОЕ ОПРЕДЕЛЕНИЕ ЦЕХА
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    addItem({ 
      ...
      department: actualDepartment // Отправляем правильный цех в корзину
    });
  };
    ...
    <>
      <div className="bg-bg-surface rounded-2xl overflow-hidden border border-border-color shadow-sm hover:shadow-md transition-shadow flex flex-col h-full relative group">
        ...
        <div className="relative block aspect-[4/3] sm:aspect-video overflow-hidden">
          ...
          {(isAdmin || isOwner) && (
             <div className="absolute top-3 right-3 flex items-center gap-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
               <button onClick={() => setEditModal(true)} className="bg-black/50 backdrop-blur-md text-white p-2 rounded-full hover:bg-black/70 transition-colors"><Edit2 className="w-4 h-4" /></button>
               <button onClick={() => setShowDeleteConfirm(true)} className="bg-red-500/80 backdrop-blur-md text-white p-2 rounded-full hover:bg-red-600 transition-colors"><Trash2 className="w-4 h-4" /></button>
             </div>
          )}
          
          <div className="absolute bottom-3 left-3 flex flex-col gap-2">
            <div className="bg-black/50 backdrop-blur-md px-3 py-1 rounded-full text-[10px] sm:text-xs font-bold text-white shadow-sm border border-white/10 uppercase tracking-wider">
              {actualDepartment}
            </div>
          </div>
        ...
        <div className="p-4 flex flex-col flex-1">
          <Link to={`/recipe/${recipe.id}`}>
            <h3 className="text-base sm:text-lg font-bold text-text-primary leading-tight mb-2 hover:text-primary transition-colors line-clamp-2">{recipe.title}</h3>
          </Link>
          <p className="text-xs sm:text-sm text-text-muted line-clamp-2 mb-4 flex-1">{recipe.description}</p>
          <button onClick={() => setShowIngredients(true)} className="self-start text-xs font-medium text-text-muted hover:text-text-primary underline underline-offset-4 mb-4">Состав блюда</button>
          
          <div className="mt-auto flex items-center justify-between gap-2 pt-3 border-t border-border-color min-h-[56px]">
            <div className="font-black text-lg sm:text-xl text-text-primary">{displayPrice} ₽</div>
            
            {cartItem ? (
              <div className="flex items-center h-10 w-[110px] bg-bg-surface-light rounded-full border border-border-color overflow-hidden">
                <button onClick={handleDecrease} className="w-10 h-full flex items-center justify-center text-text-muted hover:text-text-primary hover:bg-bg-surface transition-colors"><Minus className="w-4 h-4" /></button>
                <span className="flex-1 flex items-center justify-center font-bold text-text-primary text-sm">{cartItem.quantity}</span>
                <button onClick={handleIncrease} className="w-10 h-full flex items-center justify-center text-text-muted hover:text-text-primary hover:bg-bg-surface transition-colors"><Plus className="w-4 h-4" /></button>
              </div>
            ) : (
              <button onClick={handleAddToCart} className="inline-flex items-center justify-center h-10 bg-bg-surface-light hover:bg-primary hover:text-black text-text-primary border border-border-color px-5 rounded-full text-sm font-bold transition-all active:scale-95">В корзину</button>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-bg-surface rounded-3xl w-full max-w-md overflow-hidden shadow-2xl relative">
            <div className="p-6 border-b border-border-color flex items-center justify-between">
              ...
              <button onClick={() => setShowIngredients(false)} className="text-text-muted hover:text-text-primary transition-colors"><X className="w-6 h-6" /></button>
            </div>
                      ...
                      {ingredients.map((ing, idx) => (
                        <li key={idx} className="flex items-center justify-between text-sm">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 bg-bg-surface-light rounded-lg flex items-center justify-center shadow-sm border border-border-color"><IngredientIcon name={ing.name} className="w-4 h-4 text-primary" /></div>
            ...
            <div className="p-6 border-t border-border-color bg-bg-surface-light">
              <button onClick={(e) => { handleAddToCart(e); setShowIngredients(false); }} className="w-full bg-primary hover:bg-primary-hover text-black font-bold py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition-colors">
      ...
      {editModal && <AdminEditModal isOpen={editModal} onClose={() => setEditModal(false)} title="Редактировать блюдо" fields={[{ name: 'title', label: 'Название', type: 'text', value: recipe.title }, { name: 'description', label: 'Описание', type: 'textarea', value: recipe.description }]} onSave={handleSave} />}
      {showDeleteConfirm && (
